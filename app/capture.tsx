...
type Recognized = { name: string; type: string };

// Native has no implicit page origin the way a browser does, so a relative
// fetch("/api/...") only works on web. In dev, Metro's own host:port (the
// same address Expo Go loaded this bundle from) is the right place to send
// it; a production build would instead need `origin` configured in app.json.
...
export default function CaptureScreen() {
  ...
  // Real device location for the "기록" chip — best-effort: a denied
  // permission or a platform without reverse geocoding (web) just falls
  // back to a plain label instead of blocking capture.
  useEffect(() => {
    (async () => {
  ...
  const attemptSend = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${getApiBaseUrl()}/api/recognize`, {
        method: "POST",
      ...
      setRecognized({ name: data.name, type: data.type });
    } catch (e) {
  ...
  const register = () => {
    ...
    router.back();
    router.push({ pathname: "/share", params: { memo } });
  };
  ...
  const centerButtonDisabled =
    phase === "recognizing" || (!photoUri && !cameraReady);
